...
def extract_bus_details():
    bus_data = []
    MAX_RECORDS = 40
    URL = "https://www.redbus.in/bus-tickets/delhi-to-srinagar?fromCityId=733&toCityId=84913&fromCityName=Delhi&toCityName=Srinagar%20(Jammu%20and%20Kashmir)&busType=Any&onward=20-Feb-2025"
    ...
    try:
        ...
        scroll_pause_time = 4  # Slightly increased pause
        for _ in range(15):  
        ...
        for bus in buses[:MAX_RECORDS]:
            try:
                ...
                time.sleep(2)  # Increased delay to prevent detection
...
def store_data_in_db(bus_data):
    try:
        ...
        create_table_query = """
        CREATE TABLE IF NOT EXISTS jkrtcinfo (
        ...
        insert_query = """
        INSERT INTO jkrtcinfo (Bus_name, Bus_type, Departing_time, Duration, Reaching_time, Star_rating, Price, Seat_availability)
